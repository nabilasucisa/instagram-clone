...
import { Button, XStack, Text, ScrollView, Image, YStack, ZStack, Label } from 'tamagui'
    ...
    <SafeAreaProvider>
      <SafeAreaView>
        <TamaguiProvider config={tamaguiConfig}>
          {/* Instagram text */}
          <XStack justifyContent="space-between" alignItems='center' paddingHorizontal={15} paddingVertical={5}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <XStack paddingVertical={7} paddingHorizontal={15}>
              <XStack gap={15} alignItems='center'>
                <YStack alignItems='center'>
                  ...
                  <XStack position='absolute' bottom={0} right={0}>
                    <ZStack position='absolute' >
                      <MaterialCommunityIcons name="circle" size={30} color="white" style={{position: 'absolute',  bottom: 15, right: 0, }}/>
                      <MaterialCommunityIcons name="plus-circle" size={30} color="#0393f3" style={{position: 'absolute',  bottom: 15, right: 0, }}/>
                    </ZStack>
                  </XStack>
                ...
                <XStack gap={10}>
                  {stories.map(({ id, uri, name })=>(
                    ...
                    >
                      <LinearGradient
                        colors={[ '#cc2366', '#bc1888', '#bc1888', '#fcc353', '#f09433', '#e6683c', '#dc2743', ]}
                        start={[0, 0]}
                        end={[1, 1]}
                        borderRadius={50}
                        padding={2}
             ...
             <YStack>
                <YStack paddingVertical={7} paddingHorizontal={15}>
                  <XStack alignItems='center' justifyContent='space-between' gap={5}>
                    <MaterialCommunityIcons name='check-circle-outline' size={30} color={"#bc1888"}/>
                    <Text color={'grey'} fontSize={18}>You've seen all new posts from the past 3 days from accounts you follow.</Text>
                  </XStack>
                  <XStack paddingHorizontal={5} paddingVertical={5} alignItems='center' justifyContent='space-between'>
                    <Text color={'black'} fontSize={25}>Suggested for you</Text>
                    <Text color={'#0393f3'} fontSize={20}>Older posts</Text>
                  </XStack>
                </YStack>
             {/* FOTO NO PADDING */}
             {FEEDS_DATA.map((post) => {
                return (
                  <YStack key={post.id} paddingBottom={170}>
                    <XStack
                    ...
                    <XStack backgroundColor={"white"} paddingLeft={5}>
                      <XStack position='relative' paddingVertical={15}>
                        {post.feed.friendLikes.slice(0,3).map(({ imageUrl, name }, index) => (
                          <Image
                            key={name}
                            source={{ uri: imageUrl, width: 24, height: 24 }}
                            borderRadius={50}
                            position='absolute'
                            left={(index * 25) / 2}
                          />
                      ...
                      <Text marginLeft={2 * 25 + 10}>
                        Liked by <Text fontWeight={700}>{post.feed.friendLikes[0].name}</Text> and 9 others
                      </Text>
                    </XStack>
